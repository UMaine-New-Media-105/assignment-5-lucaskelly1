...
function setup() {
  ...
  // create 50 donuts with fixed position and random size
  for (let i = 0; i < 50; i++) {
    let x = random(width/4); // set x position to the left quarter of the canvas
    let y = random(height);
    let radius = random(10, 30); // smaller radius
    donuts.push(new Donut(x, y, radius));
  }
  
  // create 50 squares with fixed position and random size
  for (let i = 0; i < 50; i++) {
    let x = random(width/2, width); // set x position to the right half of the canvas
    let y = random(height);
    let size = random(10, 30); // smaller size
    squares.push(new Square(x, y, size, size));
  }
}
...
function draw() {
  ...
  // show donuts at fixed positions on the left side of the canvas
  for (let i = 0; i < donuts.length; i++) {
    donuts[i].show();
  }

  // show squares at fixed positions on the right side of the canvas
